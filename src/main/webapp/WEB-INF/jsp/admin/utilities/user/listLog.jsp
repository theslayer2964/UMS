...
<c:url var="formURL" value="/utilities/user_c2_log.html"/>
<html>
<head>
    <title><spring:message code="menu.system.statisticalUserByLog"/></title>
</head>
<style>
                ...
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active"><spring:message
                                code="menu.system.statisticalUserByLog"/></li>
                    </ol>
                </div>
                <h4 class="page-title"><spring:message code="menu.system.statisticalUserByLog"/></h4>
            </div>
        </div>
    </div> <!--end title-->
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <%@include file="listLogUserDetail.jsp" %>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function loadingData() {
        ...
        $('#example1').hide();
        $.ajax({
            url: '<c:url value="/admin/statistical_log_user.html" />',
            type: 'get',
            data: {
            ...
            success: function (data) {
                ...
                $('#example1').DataTable({
                    ...
                    columns: [
                        {
                            title: '<spring:message code="label.action"/>', data: "user_name",
                            render: function (code) {
                                return "<a class=\'btn-icon waves-effect waves-light btn-warning btn-sm\' onclick=\'showUserIp(" + JSON.stringify(code) + ")\'><i class='fa fa-info-circle'></i></a>";
                            }
                        },
    ...
    function showUserIp(user_name) {
        $("#loadingEvent2").show();
        $('#usernameSearch').text(user_name);
        ...
        $('#table2').hide();
        $.ajax({
            url: '<c:url value="/ajax/getViewLogIpDetail_User.html" />',
            type: 'get',
            data: {
                user_name: user_name,
                from: document.getElementById('searchNameFrom').value,
            ...
            success: function (data) {
                ...
                $('#example2').DataTable({
                    ...
                    columns: [
                        ...
                        {title: "<spring:message code="label.c2user.quantity"/>", data: "quantity"}
                    ]
                });
                $("#loadingEvent2").hide();
                $('#table2').show();
