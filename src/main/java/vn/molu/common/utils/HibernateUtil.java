 ...
 * Class HibernateUtil cung cấp các phương thức hỗ trợ xây dựng các câu truy vấn tên (named query) cho việc tìm kiếm
 * và phân trang trong Hibernate.
 */
public final class HibernateUtil {
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private static String getNameCriteria(PropertyType additionalTypeInfo, String property, Object value[],
			boolean exactMatch, boolean ignoreCase) {
		...
		if (value[1] instanceof List) {
			...
			if (exactMatch) {
				if (ignoreCase) {
					...
					buffer.append(aliasDot).append(property).append(value[0]).append(":")
							.append(getNoDotProperty(property));
				}
			} else {
				if (HibernateUtil.notIn.equals(value[0])) {
					buffer.append(aliasDot).append(property).append(notIn).append("(:")
	...
	private static String getNameCriteria(String property, Object[] value, Map<String, PropertyType> additionalTypeInfo,
			boolean exactMatch, boolean ignoreCase) {
		if (additionalTypeInfo != null && additionalTypeInfo.get(property) != null) {
	...
	public static Object[] buildNameQuery(Class clazz, Map<String, Object> propertyNameValues,
			Map<String, PropertyType> additionalTypeInfo, String orderBy, String orderDirection, boolean andSearch,
		...
		if (propertyNameValues.size() > 0) {
			...
			if (values.size() > 0) {
				return new Object[] { buffer.toString(), orderByBuffer.toString(), paramNames.toString().split("[,]"),
						values.toArray() };
			} else {
	...
	private static String getNameCriteria(String property, Object value[], int index) {
		return aliasDot + property + value[0] + ":" + (getNoDotProperty(property) + index);
	}
	...
	public static Object[] buildNameQuery(Class clazz, Map<String, Object> propertyNameValues,
			Map<String, PropertyType> additionalTypeInfo, String orderBy, String orderDirection, boolean andSearch,
			boolean exactMatch, String whereClause, boolean ignoreCase, String selectFromClause) {
