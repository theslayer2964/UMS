 ...
 * Class RequestUtil dùng để thao tác với các yêu cầu (request) và phản hồi (response) HTTP trong ứng dụng web.
 */
public final class RequestUtil {
    ...
    /*
        setCookie: Phương thức này được sử dụng để thiết lập cookie trong phản hồi (response).
    */
    ...
    * */
     ...
     * @param path
     */
    public static void setCookieDestroyOnClosed(HttpServletResponse response, String name, String value, String path) {
        if (log.isDebugEnabled()) {
            log.debug("Setting cookie '" + name + "' on path '" + path + "'");
        }
    ...
    /**
     * Convenience method to get the server's URL based on request variables not
     * contain context path.
     *
     * @param request the current request
...
//
//    @SuppressWarnings("rawtypes")
//    public static void initSearchBean4report(HttpServletRequest request, AbstractCommand command) {
//	if (command != null) {
//	    String sortExpression = request.getParameter(
//		    new ParamEncoder(command.getTableId()).encodeParameterName(TableTagParameters.PARAMETER_SORT));
//	    String sortDirection = request.getParameter(
//		    new ParamEncoder(command.getTableId()).encodeParameterName(TableTagParameters.PARAMETER_ORDER));
//	    String sPage = request.getParameter(
//		    new ParamEncoder(command.getTableId()).encodeParameterName(TableTagParameters.PARAMETER_PAGE));
//	    Integer page = 1;
//	    if (StringUtils.isNotBlank(sPage)) {
//		try {
//		    page = Integer.valueOf(sPage);
...
//	    command.setFirstItem((command.getPage() - 1) * Constants.REPORT_MAXPAGEITEMS);
