package vn.molu.webapp.pagination;
...
public class PaginationCriteria {
    ...
    public <T> void asynrCommand(AbstractCommand<T> command, DataTableRequest<T> dataTableInRQ) {
        command.setFirstItem(this.getPageNumber());
        command.setMaxPageItems(this.getPageSize());
        ...
        if(!isSortByEmpty()) {
            ...
            while (sbit.hasNext()) {
                Map.Entry<String, SortOrder> pair =  sbit.next();
                command.setSortDirection(pair.getValue() == SortOrder.ASC? Constants.SORT_ASC: Constants.SORT_DESC);
                command.setSortExpression(pair.getKey());
            }
        }
    }
